...
따라서 결과 분석에서 “시간이 적게 걸린다”만 보고 끝내지 않고, 무엇 때문에 빠른지까지 함께 파악하고자 한다.
...
동일한 키 값이 여러 개 있을 때 입력 순서가 보존되는지를 안정성으로 본다. 카운팅 정렬, 칵테일 셰이커 정렬은 일반적으로 안정적이고, 셸 정렬은 불안정하다. 
...
따라서 정렬 알고리즘을 선택할 때는 한 번에 가장 빠른 알고리즘을 고르기보다, 입력 데이터의 범위와 분포에 따라 상황에 맞는 최적의 알고리즘을 선택해야 한다. 특히 Python 버전은 소스 코드를 기계어로 컴파일하여 CPU가 직접 명령을 실행하는 C 언어와 달리, 인터프리터가 코드를 한 줄씩 해석하며 실행하는 방식으로 진행되어, C 버전보다 실행 환경이 달라질 수 있으므로, 구현 언어의 차이를 반영해 해석하는 것이 중요하다.
...
이번 보고서에선 세 가지 정렬 알고리즘의 원리, 구현, 실험 절차, 시각화 결과를 정리하였다. 그러면서, 그동안 배웠던 정렬들을 정리하면서 수업시간 배웠던 내용들을 복습하고, 더욱 구조적인 체계를 자세히 이해할 수 있었다. 특히 정렬 2개를 사용하는 카운팅 정렬은 말로만 들었을 때는 너무 추상적으로만 느껴졌는데, 이번에 보고서를 작성하면서 시각화 자료를 AI에게 부탁하여 만들었고, 이를 바탕으로 머릿속에 카운팅 정렬의 구조를 더욱 확실하게 정리할 수 있었다. 새로운 정렬인 칵테일 셰이커 정렬도 조사하며, 새로운 정렬 알고리즘에 대해 배울 수 있었다. 다른 알고리즘들과 다르게 독특한 이름을 가지고 있어 어떤 방식으로 작동하는지 궁금했는데, 칵테일을 섞듯이 배열 속 요소들 중 가장 큰 값을 오른쪽으로 한 번 이동시키고, 가장 작은 값을 왼쪽으로 한 번 이동시키는 방식으로 작동된다는 것을 이해할 수 있었다. 이러한 점에서 버블 정렬과 비슷한 점이 있다는 생각이 들었고, 다양한 알고리즘 간의 공통점과 차이점에 대해 이해할 수 있었다. 또한, 같은 정렬 알고리즘을 파이썬으로 한 번, C언어로 한 번 작성해보면서, 각각의 언어로 알고리즘을 작성하였을 때 함수, 포인터 등의 영향으로 프로그램의 형태는 변화할 수 있지만, 구조는 거의 바뀌지 않고 유지된다는 것을 직접 경험할 수 있었다. 이번 과제를 통해, 정렬 알고리즘은 어떤 문제를 풀고 있는지, 입력 데이터의 특성이 무엇인지에 따라 최적의 선택이 달라지기 때문에, 알고리즘을 평가할 때는 단순히 속도 하나만 비교하는 것이 아니라, 데이터의 성질과 구조적 특성을 함께 고려해야 한다는 것을 알게 되었다. 
