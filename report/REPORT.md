...
이번 과제를 통해 세 정렬 알고리즘을 같은 입력 생성기와 실행 인터페이스로 구현하고, 결과 배열뿐 아니라 비교 횟수·이동 횟수·실행 시간을 함께 측정해 알고리즘의 특성을 비교하고자 한다.

- 비교 대상: 셸 정렬, 카운팅 정렬, 칵테일 셰이커 정렬

시간은 운영체제와 현재 부하의 영향을 받으므로 알고리즘 실행에서 걸린 시간뿐만 아니라, 같은 환경에서 반복해 얻은 비교·이동 횟수와 입력 형태별 경향을 함께 해석하였다.

## 2. 구현 구조와 실험 인터페이스

알고리즘을 작성하기 전에 모든 구현이 같은 방식으로 호출되도록 인터페이스를 먼저 제작하였다. C에서는 `SortFunction` 함수 포인터를 사용하였고, 각 정렬 함수는 정수 배열, 원소 수, 통계 구조체의 주소를 받고 배열 자체를 정렬하였다.
...
`sort.c`에 레지스트리를 두 `main.c`는 알고리즘 이름을 하나씩 호출하지 않고, 새 알고리즘을 추가할 때 구현 파일과 레지스트리 항목을 추가하면 동일한 실험에 참여할 수 있게 하였다. `sortctx.h`의 통계 구조체도 알고리즘이 결과를 반환하는 방식과 측정 방법을 분리하도록 설계하였다. `main.c`는 입력을 보존하기 위해 작업 배열을 매번 복사하고, 정렬 뒤 `is_sorted`로 결과를 검증하는 과정을 거치도록 제작하였다.

`Makefile`의 `run`은 사람이 읽는 표를, `--csv`는 그래프가 읽는 구조화된 데이터를 만들고, `--blocks`는 `n = 128, 256, 512, 1024, 2048`로 바꾸어 입력 크기 증가 실험을 수행하는 역할이다. C 테스트와 Python 테스트는 빈 배열, 한 원소, 중복, 음수, 역순을 검사하는 기능을 가지고 있다.

## 3. C와 Python 구현의 공통점과 차이점

### 3.1 공통점

두 언어의 세 함수는 모두 입력을 오름차순으로 정렬하며 원본을 직접 바꾸지 않는 Python 함수와 달리 C 함수는 작업 배열을 받지만, 실험 프로그램이 원본을 복사한 뒤 넘기므로 알고리즘 비교 조건은 동일하다. 세 알고리즘의 핵심 단계도 같다.
...
| 통계 | `SortStats` 포인터로 비교·이동 횟수를 직접 누적 | 반환값 중심의 간단한 공개 API이며 현재 통계 구조체는 사용하지 않음 |
...
두 구현(C, Python)은 언어 런타임과 메모리 모델이 다르므로 걸린 시간을 직접 비교해서는 안 된다. 두 구현에서는 사용한 기능에서도 차이점이 있다. Python 쪽의 `list(values)`는 호출자 입력을 보호하는 장점이 있고, C 쪽의 제자리 처리는 메모리와 실행 비용을 더 명시적으로 통제할 수 있다는 차이가 있다.
