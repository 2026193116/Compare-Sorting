...
복잡도 그래프는 각 알고리즘이 입력 크기 증가에 따라 얼마나 빠르게 비용이 증가하는지를 보여 준다. 셸 정렬과 카운팅 정렬, 칵테일 셰이커 정렬은 같은 `n`에 대해서도 각각 다른 기울기를 보인다는 점에서 차이가 있다.

#### 5.2.2 입력 형태에 따른 실행 시간

![입력 형태별 실행 시간](input-time.svg)

입력 형태에 따른 알고리즘의 작동 시간을 확인할 수 있으며, 정렬된 배열은 거의 비용이 낮은 반면, 역순 또는 무작위 배열은 비용이 크게 증가할 수 있다는 것을 엿볼 수 있다.

#### 5.2.3 입력 형태에 따른 비교 횟수

![입력 형태별 비교 횟수](input-comparisons.svg)

비교 횟수는 각 알고리즘의 핵심 비용을 보여 준다. 예를 들어, 비교가 반복적으로 일어나는 버블 계열의 알고리즘(ex. 칵테일 셰이킹 정렬)은 입력이 불리한 경우 비교 횟수가 급증한다.

#### 5.2.4 입력 형태에 따른 이동 횟수

![입력 형태별 이동 횟수](input-moves.svg)

이동 횟수는 교환이 얼마나 자주 일어나는지를 보여 주며 정렬이 중복하거나 역순인 경우, 교환이 반복되며 이동 횟수가 큰 폭으로 증가한다.

#### 5.2.5 입력 크기 증가에 따른 시간 변화

![입력 크기별 실행 시간](scale-time.svg)

그래프를 통해 입력 크기가 커질수록 알고리즘이 얼마나 급격히 느려지는지를 확인할 수 있다. 이때, 가장 중요한 것은 복잡도가 비슷해 보일 때도 실제 실행 시간은 입력 유형과 구현 세부사항에 따라 다르게 나타날 수 있다는 것이다.
...
따라서 정렬 알고리즘을 선택할 때는 한 번에 가장 빠른 알고리즘을 고르기보다, 입력 데이터의 범위와 분포에 따라 상황에 맞는 최적의 알고리즘이 달라진다는 점을 고려해야 한다.
