...
카운팅 정렬은 입력값의 범위를 기준으로 각 값의 출현 횟수를 세고, 누적 개수를 이용해 각 원소의 최종 위치를 계산하는 정렬 알고리즘이다. 최솟값과 최댓값의 차이를 `k = max - min + 1`이라고 하면 시간 복잡도는 `O(n + k)`, 추가 공간 복잡도는 `O(n + k)`이다.

카운팅 정렬은 안정 정렬이다. 안정 정렬이란 정렬 기준값이 같은 원소들의 입력 순서가 정렬 후에도 유지되는 것을 의미한다. 카운팅 정렬은 누적 개수로 원소의 위치를 계산하고, 같은 값을 가진 원소를 입력 순서에 맞게 배치하여 이 성질을 유지한다. 따라서 같은 키를 가진 레코드의 상대적 순서를 보존해야 하는 상황에서도 사용할 수 있다.
...
카운팅 정렬의 안정성은 동일한 정렬 키를 가진 레코드의 상대적 순서를 보존한다는 일반적인 기준으로 판단한다. 이는 특정 입력이나 저장소의 구현 세부 조건을 전제로 한 설명이 아니다.
...
카운팅 정렬은 값의 범위가 제한된 정수 입력에서 `O(n+k)` 시간에 동작하며, 동일한 키를 가진 원소의 상대적 순서를 보존하는 안정 정렬이다.
