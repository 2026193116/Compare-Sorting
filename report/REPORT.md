...
이번 과제를 통해 셸 정렬, 카운팅 정렬, 칵테일 셰이커 정렬, 총 세 개의 정렬 알고리즘을 C언어와 파이썬, 총 2개의 언어를 바탕으로 공통된 인터페이스를 사용하여 구현하고, 입력 형태와 크기에 따른 비교 횟수·이동 횟수·실행 시간을 함께 측정하여 각 알고리즘의 특성을 비교하였다.
...
이번 과제를 통해 C언어와 파이썬을 활용하여 코드를 작성하고, 정렬 함수의 작동이 제대로 되는지를 확인하는 것보다, 전체적으로 알고리즘의 진행 방식과 동일한 조건에서 서로 다른 알고리즘이 차이점을 가지고 있는지 학습하고자 한다. 특히 알고리즘의 성능은 입력의 분포와 배열의 상태에 따라 크게 달라진다는 점을 고려하여, 정렬을 비교할 때는 단순히 한 번의 입력에 대한 결과만 보지 않고, 다양한 입력 조건에 따른 차이를 살펴보았다.
...
입력 생성 방식, 시간 측정 방식, 통계 집계 방식이 조금만 달라도 결과가 서로 다르게 보이기 때문에 세 알고리즘을 각각 따로 구현하는 것만으로는 비교가 어렵다고 생각하였고, 이를 해결하고자 알고리즘의 구현에서 같은 인터페이스를 공유하도록 설계했다.
