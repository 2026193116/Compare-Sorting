...
`make run`은 C 구현의 비교 표와 검증 결과를 출력합니다. `make charts`는 실험 결과를 `report/results.csv`에 저장하고 SVG 그래프를 생성합니다.
...
| --- | --- | --- | --- | --- |
| 셸 정렬 | 배운 정렬 | 초기 `gap = floor(n/4)`, 이후 절반씩 감소할 때 `O(n^1.5)`로 분석 | `O(1)` | 불안정 |
| 카운팅 정렬 | 배운 정렬 | `O(n + k)` | `O(n + k)` | 안정 구현 가능 |
| 칵테일 셰이커 정렬 | 배우지 않은 정렬 | `O(n²)`, 최선 `O(n)` | `O(1)` | 안정 |

여기서 셸 정렬은 `gap = floor(n/4)`에서 시작하여 `gap /= 2`를 반복하고, 마지막에 `gap = 1`인 삽입 정렬 pass를 수행합니다. 따라서 `n/4`는 한 번의 gap이 아니라 전체 gap 수열의 시작값입니다. `k`는 입력의 최댓값과 최솟값 사이의 값 범위입니다.
